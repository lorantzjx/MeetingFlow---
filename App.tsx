...
const AppContent = () => {
  ...
  const [settings, setSettings] = useState<AppSettings>(() => {
    ...
    if (parsed && Array.isArray(parsed.templates)) {
      // 确保 positions 存在
      if (!parsed.positions) parsed.positions = ['总', '工', '处', '部', '无'];
      return parsed;
    }
    return {
  ...
  useEffect(() => {
    localStorage.setItem('mf_contacts', JSON.stringify(contacts));
  }, [contacts]);
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col shrink-0">
        <div className="p-6 flex items-center space-x-2">
          <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-lg shadow-blue-100">M</div>
